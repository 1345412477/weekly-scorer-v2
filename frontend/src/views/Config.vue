...
function resetConfig() {
  ...
  promptTemplate.value = `# å¨æ¥è¯åç³»ç»æç¤ºè¯

## è§è²è®¾å®
ä½ æ¯ä¸ä½ä¸ä¸ãå®¢è§çå·¥ä½æ¥åè¯å®¡ä¸å®¶ãè¯·æ ¹æ®ä»¥ä¸è¯åç»´åº¦å¯¹åå·¥å¨æ¥è¿è¡ç»¼åè¯ä¼°ã

## è¯ååå
1. å®¢è§å¬æ­£ï¼åºäºå¨æ¥åå®¹è¿è¡è¯ä»·ï¼é¿åä¸»è§èæ­
2. é¼å±éåï¼å¯¹ææ°æ®æ¯æãå¯éåææçåå®¹ç»äºæ´é«è¯ä»·
3. å³æ³¨é­ç¯ï¼éè§"è®¡åâæ§è¡âç»æ"çå®æ´é­ç¯
4. æä¾å»ºè®¾æ§ï¼è¯è¯­åå»ºè®®åºå·ä½ãå¯æä½

## è¯åç»´åº¦
  1. å·¥ä½åé¦æ·±åº¦ï¼æ»¡å14åï¼æé«å14åï¼æä½å8åï¼èæ ¸åå®¹ï¼é®é¢åç°+åæ+è§£å³æ¹æ¡ï¼
  2. è¿åº¦èç¹æç¡®ï¼æ»¡å13åï¼æé«å13åï¼æä½å7åï¼èæ ¸åå®¹ï¼é¡¹ç®æ¯å¦ææç¡®è¿åº¦/èç¹ï¼
  3. è®¡åå¯è¡æ§ï¼æ»¡å10åï¼æé«å10åï¼æä½å6åï¼èæ ¸åå®¹ï¼ä¸å¨è®¡åæ¯å¦å·ä½å¯æ§è¡ï¼
  4. å·¥ä½è¿ç»­æ§ï¼æ»¡å13åï¼æé«å13åï¼æä½å7åï¼èæ ¸åå®¹ï¼æ¯å¦æ¿æ¥ä¸å¨è®¡åä¸æé­ç¯ï¼

## è¯åæ å
- æ¯ä¸ªç»´åº¦ç¬ç«æåï¼ä¸è¶è¿è¯¥ç»´åº¦æ»¡å
- ç»¼åè¯å = åç»´åº¦åæ°ç´æ¥ç¸å ï¼28-50åï¼
- ç­çº§ååï¼ä¼(â¥43)ãè¯(â¥38)ãä¸è¬(â¥33)ãå·®(â¥28)

## è¾åºè¦æ±
è¯·ä»¥ JSON æ ¼å¼è¿åè¯åç»æï¼åå«ï¼
- dimension_scores: åç»´åº¦å¾ååè¯è¯­ï¼å«nameãscoreãmaxãcommentï¼
- total_score: ç»¼åå¾åï¼28-50åï¼
- grade: ç­çº§ï¼ä¼/è¯/ä¸è¬/å·®ï¼
- comment: æ»ä½è¯è¯­ï¼100å­ä»¥åï¼
- suggestion: æ¹è¿å»ºè®®ï¼å·ä½å¯æ§è¡ï¼

## å¨æ¥åå®¹
{content}`
  reportPrompt.value = `# å¨æ¥è¯åæç¤ºè¯

è¯·æ ¹æ®åå·¥æäº¤çå¨æ¥å¨ 28-50 åèå´è¿è¡è¯åã
...
## ç­çº§åå
ä¼(â¥43)ãè¯(â¥38)ãä¸è¬(â¥33)ãå·®(â¥28)

## è¾åºè¦æ±
è¯·ä»¥ JSON æ ¼å¼è¿åï¼
- dimension_scoresï¼æ¯é¡¹å«name/score/max/commentï¼
- total_scoreï¼åç»´åº¦ç¸å ï¼
- gradeï¼ä¼/è¯/ä¸è¬/å·®ï¼
- commentï¼æ»ä½è¯è¯­ï¼
- suggestionï¼æ¹è¿å»ºè®®ï¼

## å¨æ¥åå®¹
{content}`
  attendancePrompt.value = `# èå¤è¯åæç¤ºè¯
...
- commentï¼ç®ç­ç¹è¯ï¼`
  chatPrompt.value = `# æ²éä¸ä¸å¨å°ç»è¯åæç¤ºè¯
...
- scoreï¼0-100 çæ°å¼ï¼
- commentï¼ç®ç­ç¹è¯ï¼`
  businessSummaryPrompt.value = `ä½ æ¯ä¸ä½èµæ·±çé¡¹ç®ç®¡çåæå¸ï¼æé¿ä»å¨æ¥ä¸­æç¼é¡¹ç®å¨æ¯ãå½å¹¶å­ä»»å¡ãè¯å«éç¹é¡¹ç®å¹¶è¯ä¼°è¿åº¦ã

## ä»»å¡ç®æ 

æ ¹æ®é¨é¨åå·¥æäº¤çå¨æ¥åå®¹ï¼æ**é¡¹ç®ç»´åº¦**è¿è¡é«åº¦å½å¹¶ï¼å°é¶æ£çå­ä»»å¡/åè½ç¹/å·¥ä½é¡¹èç±»ä¸ºå®æ´çé¡¹ç®ï¼åºå**ä¸å¨å·²å®æ**ä¸**æ¬å¨è¿è¡ä¸­**çé¡¹ç®ã

## æ ¸å¿ååï¼åå½å¹¶ï¼åè¾åº

å¨æ¥ä¸­ç \`### N. xxx\` æ é¢éå¸¸æ¯**å­ä»»å¡æåè½æ¨¡å**ï¼ä¸æ¯é¡¹ç®åãä½ å¿é¡»å°è¿äºå­ä»»å¡å½å¹¶å°æå±é¡¹ç®ä¸ã

### å½å¹¶è§åï¼å¿é¡»ä¸¥æ ¼éµå®ï¼

1. **åä¸ç³»ç»çå­æ¨¡å â å½å¹¶ä¸ºä¸ä¸ªé¡¹ç®**
   - ä¾ï¼ãAIè¯åå¼æããç³»ç»çæ§ããå®å¨å åºããæ§è½ä¼åãâ å½å¹¶ä¸ºãèå¤è¯åç³»ç»ã
   - ä¾ï¼ãèµ¢ç­å°ç¨åºããæ¶æ¯æ¨éããæ§è½ä¼åããååæµè¯ãâ å½å¹¶ä¸ºãèµ¢ç­å°ç¨åºã
   - ä¾ï¼ãåç«¯å¼åããä»£ç ä¼åããBugä¿®å¤ãâ å½å¹¶ä¸ºæå±é¡¹ç®åï¼å¦ãèå¤è¯åç³»ç»ãæãèµ¢ç­å°ç¨åºãï¼

2. **åä¸å®¢æ·/äº§åçä¸åå·¥ä½ â å½å¹¶ä¸ºä¸ä¸ªé¡¹ç®**
   - ä¾ï¼ãYYå®¢æ·ä¸çº¿å®æ½ããYYå®¢æ·æä½å¹è®­ããå®¢æ·ååºæºå¶å»ºè®¾ãâ å½å¹¶ä¸ºãYYå®¢æ·ä¸çº¿é¡¹ç®ã
   - ä¾ï¼ãXXå®¢æ·è¿ç»´æ¯æãâ å½å¹¶ä¸ºãXXå®¢æ·è¿ç»´é¡¹ç®ã

3. **åä¸äº§åçå½å¨æçå·¥ä½ â å½å¹¶ä¸ºä¸ä¸ªé¡¹ç®**
   - ä¾ï¼ãäº§åè§åããç¨æ·è°ç ããååè®¾è®¡ããéæ±ææ¡£ãâ å½å¹¶ä¸ºãäº§åè§åä¸è°ç ã

4. **éç¨/æé¡¹å·¥ä½ â å½å¹¶ä¸ºãåºç¡å»ºè®¾ä¸ä¼åã**
   - ä¾ï¼ãææ¡£æ´çããå­¦ä¹ å¹è®­ããææ¯ææ¡£ããä»£ç è¯å®¡ãâ å½å¹¶ä¸ºãåºç¡å»ºè®¾ä¸ä¼åã
   - è¿ç±»å·¥ä½éå¸¸ä¸æ¶åå·ä½é¡¹ç®äº¤ä»ï¼ä½ä¸ºååºåç±»

5. **è·¨äººååä½ â å¿é¡»åå¹¶ä¸ºä¸æ¡**
   - ä¸ååå·¥åä¸åä¸é¡¹ç®çä¸åæ¨¡åï¼å¿é¡»åå¹¶ä¸ºä¸æ¡é¡¹ç®è®°å½ï¼persons ååºææåä¸è

### å½å¹¶ç¤ºä¾

åè®¾ç åé¨æä»¥ä¸å¨æ¥åå®¹ï¼
- åå·¥Aï¼### 1. AIè¯åå¼æ / ### 2. ç³»ç»çæ§ / ### 3. å®å¨å åº
- åå·¥Bï¼### 1. èå¤è¯åç³»ç» / ### 2. ä¸å¡çåè½ / ### 3. ææ¯ææ¡£
- åå·¥Cï¼### 1. åç«¯å¼å / ### 2. ä»£ç ä¼å / ### 3. Bugä¿®å¤

æ­£ç¡®å½å¹¶ç»æï¼2ä¸ªé¡¹ç®ï¼ï¼
- ãèå¤è¯åç³»ç»ãï¼åå«AIè¯åå¼æãç³»ç»çæ§ãå®å¨å åºãä¸å¡çåè½ãåç«¯å¼åãä»£ç ä¼åãBugä¿®å¤ï¼A+B+Cåä¸ï¼
- ãåºç¡å»ºè®¾ä¸ä¼åãï¼åå«ææ¯ææ¡£ï¼Båä¸ï¼

éè¯¯åæ³ï¼7ä¸ªç¬ç«é¡¹ç®ï¼ï¼
- AIè¯åå¼æãç³»ç»çæ§ãå®å¨å åºãèå¤è¯åç³»ç»ãä¸å¡çåè½ãåç«¯å¼åãä»£ç ä¼å... â è¿æ¯æå­ä»»å¡å½é¡¹ç®

## è¾åºæ ¼å¼

è¯·ä¸¥æ ¼æç§ä»¥ä¸ JSON æ ¼å¼è¾åºï¼ä¸è¦åå«å¶ä»åå®¹ï¼
...
| å­æ®µ | è¯´æ |
|------|------|
| **name** | å½å¹¶åçé¡¹ç®åç§°ï¼ä¸è¶è¿15å­ï¼ãä¸æ¯å­ä»»å¡åï¼æ¯æå±çç³»ç»/äº§å/å®¢æ·é¡¹ç®å |
| **progress** | è¿åº¦ç¾åæ¯ï¼0-100ï¼ãè¯ä¼°æ åï¼å·²å®æ/å·²ä¸çº¿/å·²äº¤ä»=100ï¼èè°/æµè¯ä¸­=70-90ï¼å¼åä¸­=40-70ï¼è®¾è®¡/è°ç ä¸­=10-30ï¼æªå¯å¨=0 |
| **highlight** | æ¯å¦éç¹é¡¹ç®ï¼true/falseï¼ãæ»¡è¶³ä»»ä¸æ¡ä»¶å³ä¸ºéç¹ï¼â è·¨äººååä½ï¼â¥2äººï¼â¡æ ¸å¿ä¸å¡/è¥æ¶ç¸å³ç³»ç» â¢æ¶åæ¶æåçº§æææ¯æ»å â£ææç¡®éç¨ç¢äº¤ä» |
| **summary** | ç²¾ç¼æè¿°ï¼30-80å­ï¼ï¼å¿é¡»åå«ï¼â åäºä»ä¹ â¡å³é®ææ/æ°æ® å½åç¶æãé¿åç©ºæ³æè¿° |
| **persons** | åä¸è¯¥é¡¹ç®çææäººåå§åï¼å»éï¼ |

## è¾åºçº¦æ

1. æ¯ä¸ªå¨æçé¡¹ç®æ°éæ§å¶å¨ **2-5ä¸ª**ï¼è¶è¿è¯´æå½å¹¶ä¸å¤
2. ç¦æ­¢å°å­ä»»å¡/åè½æ¨¡åä½ä¸ºç¬ç«é¡¹ç®è¾åº
3. éç¨/æé¡¹å·¥ä½ç»ä¸å½å¥ãåºç¡å»ºè®¾ä¸ä¼åã
4. è¥æå¨ææ ææé¡¹ç®ä¿¡æ¯ï¼å¯¹åºæ°ç»è¿åç©ºæ°ç» \`[]\`

## ä¸ä¸æä¿¡æ¯

- é¨é¨åç§°ï¼{department}
- ç»è®¡å¨æï¼{week_label}
